...
const sessionLogScanner_1 = require("./sessionLogScanner");
const checkpointPayload_1 = require("./checkpointPayload");
const checkpoint_1 = require("./checkpoint");
const repoRouter_js_1 = require("./repoRouter.js");
...
class SessionLogWatcher {
     ...
     */
    checkpointedActions = new Set();
    static MAX_CHECKPOINTED_ACTIONS = 3000;
    /** Timestamp of last sessionIds refresh, for cooldown. */
    lastSessionIdRefresh = 0;
    static SESSION_ID_REFRESH_COOLDOWN_MS = 10_000;
    /** Discovered git repositories in the workspace. */
    repos = [];
    /** Disposable for the git extension onDidOpenRepository subscription. */
    gitApiDisposable = null;
    constructor(workspacePath, scanner) {
        this.workspacePath = workspacePath;
        this.scanner = scanner ?? new sessionLogScanner_1.SessionLogScanner();
    }
    ...
    initRepoDiscovery() {
        ...
        const git = gitExtension.isActive
            ...
            : null;
        if (!git) {
            console.warn("[git-ai-kiro] Git API not active, using findGitRoot fallback");
            this.repos = this.discoverReposFallback();
            return;
        }
        // Map existing repositories to RepoInfo, filtering out invalid repos
        this.repos = git.repositories
            .map((repo) => ({ rootPath: (0, repoRouter_js_1.normalizePath)(repo.rootUri.fsPath) }))
            .filter((repo) => {
        ...
        this.gitApiDisposable = git.onDidOpenRepository((repo) => {
            const newRoot = (0, repoRouter_js_1.normalizePath)(repo.rootUri.fsPath);
            // Avoid duplicates
            if (!this.repos.some((r) => (0, repoRouter_js_1.normalizePath)(r.rootPath) === newRoot)) {
                this.repos.push({ rootPath: newRoot });
                console.log(`[git-ai-kiro] New repository discovered: ${newRoot}, total repos: ${this.repos.length}`);
            }
        });
        ...
        if (this.repos.length === 0) {
            ...
            console.log(`[git-ai-kiro] Repo discovery: found ${this.repos.length} repo(s) via git extension`);
        }
    }
    /**
     ...
     */
    discoverReposFallback() {
        // 1. workspace 本身是 git repo 或其子目录
        const gitRoot = (0, gitUtils_1.findGitRoot)(this.workspacePath);
        if (gitRoot) {
            console.log(`[git-ai-kiro] findGitRoot found: ${gitRoot}`);
            return [{ rootPath: (0, repoRouter_js_1.normalizePath)(gitRoot) }];
        }
        // 2. workspace 是 git 项目的父目录，扫描子目录
        const subRepos = (0, gitUtils_1.findGitReposInDir)(this.workspacePath);
        if (subRepos.length > 0) {
            console.log(`[git-ai-kiro] Found ${subRepos.length} git repo(s) in workspace subdirectories: ${subRepos.join(", ")}`);
            return subRepos.map((r) => ({ rootPath: (0, repoRouter_js_1.normalizePath)(r) }));
        }
        // 3. 最终 fallback
        console.warn("[git-ai-kiro] No git repo found, falling back to workspace root");
        return [{ rootPath: (0, repoRouter_js_1.normalizePath)(this.workspacePath) }];
    }
    /**
    ...
    async start() {
        try {
            ...
            const agentDir = sessionLogScanner_1.SessionLogScanner.resolveAgentDir();
            ...
            try {
                ...
                console.warn(`[git-ai-kiro] Agent Dir does not exist, SessionLogWatcher not starting: ${agentDir}`);
                return;
            }
            // Read session IDs for the current workspace
            this.sessionIds = await this.scanner.getWorkspaceSessionIds(this.workspacePath);
            console.log(`[git-ai-kiro] SessionLogWatcher found ${this.sessionIds.size} session ID(s) for workspace`);
            ...
            try {
                ...
                console.warn(`[git-ai-kiro] Failed to read Agent Dir: ${agentDir}`, error);
                return;
            }
            ...
            console.log(`[git-ai-kiro] SessionLogWatcher started with ${this.watchers.length} watcher(s)`);
        }
        catch (error) {
            console.error(`[git-ai-kiro] SessionLogWatcher.start() failed:`, error);
        }
    }
    /**
    ...
    watchParentDir(parentPath) {
        try {
            const watcher = fs.watch(parentPath, (eventType, filename) => {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        if (this.watchedExecLogDirs.has(subPath))
                            return;
                        console.log(`[git-ai-kiro] New execution log directory detected: ${subPath}`);
                        this.watchExecLogDir(subPath);
                        await this.snapshotExistingFiles(subPath);
                    }
                    catch {
            ...
            watcher.on("error", (error) => {
                console.warn(`[git-ai-kiro] fs.watch error on parent dir ${parentPath}:`, error);
            });
            this.watchers.push(watcher);
        }
        catch (error) {
            console.warn(`[git-ai-kiro] Failed to watch parent dir ${parentPath}:`, error);
        }
    }
    /**
     * Set up fs.watch on a single 414d* execution log directory.
     */
    watchExecLogDir(dirPath) {
        if (this.watchedExecLogDirs.has(dirPath))
            return;
        try {
            ...
            watcher.on("error", (error) => {
                console.warn(`[git-ai-kiro] fs.watch error on ${dirPath}:`, error);
            });
            this.watchers.push(watcher);
            this.watchedExecLogDirs.add(dirPath);
            console.log(`[git-ai-kiro] Watching execution log directory: ${dirPath}`);
        }
        catch (error) {
            console.warn(`[git-ai-kiro] Failed to set up fs.watch on ${dirPath}:`, error);
        }
    }
    /**
     ...
     */
    async snapshotExistingFiles(dirPath) {
        try {
            ...
            console.log(`[git-ai-kiro] Snapshot: recorded ${recorded} existing files in ${path.basename(dirPath)}`);
        }
        catch (error) {
            console.warn(`[git-ai-kiro] Snapshot failed for ${dirPath}:`, error);
        }
    }
    /**
     ...
     */
    async checkAndProcess(filePath) {
        try {
            let stat;
            try {
                stat = await fs.promises.stat(filePath);
            ...
            console.log(`[git-ai-kiro] File changed: ${path.basename(filePath)}, ` +
            ...
            console.error(`[git-ai-kiro] Error checking/processing file: ${filePath}`, error);
        }
    }
    /**
     ...
     */
    async processExecutionLog(filePath) {
        try {
            // Parse the execution log file
            const parseResult = await this.scanner.parseExecutionLogFile(filePath);
            if (!parseResult) {
                console.log(`[git-ai-kiro] Skipped (parse returned null): ${path.basename(filePath)}`);
                return;
            }
            console.log(`[git-ai-kiro] Parsed ${path.basename(filePath)}: format=${parseResult.format}, ` +
            ...
            if (this.sessionIds.size > 0 && parseResult.chatSessionId !== undefined) {
                if (!this.sessionIds.has(parseResult.chatSessionId)) {
                    ...
                    if (now - this.lastSessionIdRefresh > SessionLogWatcher.SESSION_ID_REFRESH_COOLDOWN_MS) {
                        ...
                        if (refreshed.size > this.sessionIds.size) {
                            console.log(`[git-ai-kiro] Refreshed session IDs: ${this.sessionIds.size} → ${refreshed.size}`);
                            this.sessionIds = refreshed;
                        }
                    }
                    // Check again after refresh
                    if (!this.sessionIds.has(parseResult.chatSessionId)) {
                        console.log(`[git-ai-kiro] Skipped (sessionId mismatch): ${path.basename(filePath)}, ` +
                ...
                console.log(`[git-ai-kiro] Skipped (no chatSessionId): ${path.basename(filePath)}`);
                return;
            }
            ...
            {
                const ignPats = (0, checkpoint_1.getIgnorePatterns)();
                // 按 repo 分组（复用 groupActionsByRepo 逻辑）
                const { groups: delGroups } = (0, repoRouter_js_1.groupActionsByRepo)(writeActions, this.repos, this.workspacePath);
                for (const group of delGroups) {
                    const allEarliest = new Map();
                    const allLatest = new Map();
                    for (const wa of group.actions) {
                        ...
                        if (!existing) {
                            ...
                            if (ct < (ear.emittedAt ?? 0))
                                allEarliest.set(wa.filePath, wa);
                        }
                    }
                    let aiNetDel = 0;
                    for (const [fp, latest] of allLatest) {
                        if (ignPats.length > 0 && matchesIgnorePatternSafe(fp, ignPats))
                            continue;
            ...
            console.log(`[git-ai-kiro] Processing ${writeActions.length} write action(s) from: ${filePath}`);
            ...
            for (const wa of writeActions) {
                ...
                console.log(`[git-ai-kiro]   action: ${wa.actionType}, file: ${wa.filePath}, ` +
            ...
            if (newActions.length === 0) {
                console.log(`[git-ai-kiro] Skipped (all ${writeActions.length} actions already checkpointed): ${path.basename(filePath)}`);
                return;
            }
            if (newActions.length < writeActions.length) {
                console.log(`[git-ai-kiro] Filtered: ${writeActions.length} total → ${newActions.length} new action(s)`);
            }
            const ignorePatterns = (0, checkpoint_1.getIgnorePatterns)();
            // Group actions by repository for per-repo dispatch
            // First, filter actions to only include files under the current workspace
            const workspacePrefix = (0, repoRouter_js_1.normalizePath)(this.workspacePath).toLowerCase();
            const workspaceFilteredActions = newActions.map((action) => {
                let fp = action.filePath;
                // 如果 filePath 是绝对路径，转为 workspace-relative
                const normalizedFp = (0, repoRouter_js_1.normalizePath)(fp);
                const wsNorm = (0, repoRouter_js_1.normalizePath)(this.workspacePath);
                if (normalizedFp.toLowerCase().startsWith(wsNorm.toLowerCase() + "/") || normalizedFp.toLowerCase().startsWith(wsNorm.toLowerCase() + "\\")) {
                    ...
                    for (const repo of this.repos) {
                        const repoNorm = (0, repoRouter_js_1.normalizePath)(repo.rootPath);
                        if (normalizedFp.toLowerCase().startsWith(repoNorm.toLowerCase() + "/") || normalizedFp.toLowerCase().startsWith(repoNorm.toLowerCase() + "\\")) {
                            ...
                            const repoRelToWs = (0, repoRouter_js_1.normalizePath)(path.relative(this.workspacePath, repo.rootPath));
                ...
                const absPath = (0, repoRouter_js_1.normalizePath)(path.resolve(this.workspacePath, action.filePath)).toLowerCase();
                if (!absPath.startsWith(workspacePrefix)) {
                    ...
                    const inRepo = this.repos.some((repo) => {
                        const repoPrefix = (0, repoRouter_js_1.normalizePath)(repo.rootPath).toLowerCase();
                        ...
                        try {
                            ...
                            if (folders) {
                                for (const f of folders) {
                                    const folderPrefix = (0, repoRouter_js_1.normalizePath)(f.uri.fsPath).toLowerCase();
            ...
            const { groups, orphans } = (0, repoRouter_js_1.groupActionsByRepo)(workspaceFilteredActions, this.repos, this.workspacePath);
            // Log orphan files (actions that don't match any discovered repo)
            // 对于 orphan 文件，尝试动态发现其所属的 git repo 并重新路由
            if (orphans.length > 0) {
                const newReposDiscovered = [];
                for (const orphan of orphans) {
                    const absPath = (0, repoRouter_js_1.normalizePath)(path.resolve(this.workspacePath, orphan.filePath));
                    const dirPath = path.dirname(absPath);
                    const gitRoot = (0, gitUtils_1.findGitRoot)(dirPath);
                    if (gitRoot) {
                        const normalizedRoot = (0, repoRouter_js_1.normalizePath)(gitRoot);
                        // 避免重复添加
                        if (!this.repos.some((r) => (0, repoRouter_js_1.normalizePath)(r.rootPath).toLowerCase() === normalizedRoot.toLowerCase())) {
                ...
                if (newReposDiscovered.length > 0) {
                    const reGrouped = (0, repoRouter_js_1.groupActionsByRepo)(orphans, this.repos, this.workspacePath);
                    ...
                    try {
                        ...
                        if (segments.length >= 2) {
                            ...
                            try {
                                ...
                                const candidateGitRoot = (0, gitUtils_1.findGitRoot)(path.dirname(candidatePath));
                                if (candidateGitRoot) {
                                    const normalizedRoot = (0, repoRouter_js_1.normalizePath)(candidateGitRoot);
                                    if (!this.repos.some((r) => (0, repoRouter_js_1.normalizePath)(r.rootPath).toLowerCase() === normalizedRoot.toLowerCase())) {
                                        this.repos.push({ rootPath: normalizedRoot });
                                        console.log(`[git-ai-kiro] Dynamically discovered sibling repo: ${normalizedRoot}`);
                                    }
                                    // 将此 action 移到正确的 group（Windows 大小写不敏感匹配）
                                    const normalizedCandidate = (0, repoRouter_js_1.normalizePath)(candidatePath);
                                    const normalizedGitRoot = (0, repoRouter_js_1.normalizePath)(candidateGitRoot);
                                    let repoRelPath;
                                    ...
                                    let targetGroup = groups.find((g) => (0, repoRouter_js_1.normalizePath)(g.repoPath).toLowerCase() === normalizedRoot.toLowerCase());
                                    if (!targetGroup) {
            ...
            if (validGroups.length === 0) {
                console.log(`[git-ai-kiro] Skipped (no repo groups after grouping): ${path.basename(filePath)}`);
                return;
            }
            ...
            for (const group of validGroups) {
                ...
                try {
                    ...
                    const humanPayload = this.buildHumanPayload(repoActions, parseResult.chatSessionId, ignorePatterns, repoPath);
                    if (humanPayload) {
                        console.log(`[git-ai-kiro] Sending human checkpoint for repo ${repoPath}: ${JSON.stringify({
                            type: humanPayload.type,
                            files: Object.keys(humanPayload.dirty_files),
                        })}`);
                        const humanOk = await (0, checkpoint_1.callCheckpointAgentV1)(repoPath, humanPayload);
                        if (!humanOk) {
                            console.error(`[git-ai-kiro] Human checkpoint failed for repo ${repoPath}: ${filePath}`);
                        }
                    }
                    // Step 2: Send AI checkpoint (modifiedContent as post-edit content)
                    const aiPayload = await (0, checkpointPayload_1.buildCheckpointPayload)(repoPath, repoActions, parseResult.chatSessionId, ignorePatterns);
                    ...
                    console.log(`[git-ai-kiro] Sending AI checkpoint for repo ${repoPath}: edited_filepaths=${JSON.stringify(finalAiPayload.edited_filepaths)}, ` +
                        `dirty_files keys=${JSON.stringify(Object.keys(finalAiPayload.dirty_files))}`);
                    const aiOk = await (0, checkpoint_1.callCheckpointAgentV1)(repoPath, finalAiPayload);
                    if (aiOk) {
                        console.log(`[git-ai-kiro] AI checkpoint succeeded for repo ${repoPath}: ${filePath}`);
                        anySuccess = true;
                        ...
                        console.error(`[git-ai-kiro] AI checkpoint failed for repo ${repoPath}: ${filePath}`);
                        anyFailure = true;
                    }
                }
                catch (error) {
                    console.error(`[git-ai-kiro] Checkpoint error for repo ${repoPath}: ${filePath}`, error);
                    anyFailure = true;
            ...
            console.error(`[git-ai-kiro] processExecutionLog failed for: ${filePath}`, error);
        }
    }
    /**
     ...
     */
    buildHumanPayload(writeActions, chatSessionId, ignorePatterns, repoPath) {
        const dirtyFiles = {};
 ...
 */
function matchesIgnorePatternSafe(filePath, patterns) {
    try {
        return (0, checkpoint_1.matchesIgnorePattern)(filePath, patterns);
    }
    catch {
        return false;
    }
}
//# sourceMappingURL=sessionLogWatcher.js.map