...
exports.reportUserLogin = reportUserLogin;
exports.stopUserSync = stopUserSync;
...
const qClientWatcher_1 = require("./qClientWatcher");
...
function reportUserLogin() {
    if (watcher)
        return;
    isFirst = true;
    watcher = new qClientWatcher_1.QClientLogWatcher((info) => maybeDoUserSync(info.userId));
    watcher.start();
 ...
 */
async function maybeDoUserSync(userId) {
    if (inflight) {
...
async function doUserSync(userId) {
    ...
    console.log(`[git-ai-kiro] userSync payload: ${JSON.stringify(payload)} → ${apiConfig_1.USER_SYNC_URL}`);
    writeDebugLog(payload);
    try {
        const status = await doPost(apiConfig_1.USER_SYNC_URL, "", JSON.stringify(payload));
 ...
 * 已经是裸 UUID 的输入原样返回。
 */
function stripIdentityStorePrefix(raw) {
    if (!raw)
        return "";
 ...
 */
function getEmailFromKiroCli() {
    try {
        const output = (0, node_child_process_1.execFileSync)("kiro-cli", ["whoami"], {
 ...
 * 没有记录返回 null。
 */
function getLastUserSyncTimestamp() {
    try {
        const folders = vscode.workspace.workspaceFolders;
        if (!folders)
            return null;
...
// ==================== 日志清理 ====================
/** 清理日志文件中超过指定天数的行 */
function cleanOldLines(filePath, days) {
    try {
