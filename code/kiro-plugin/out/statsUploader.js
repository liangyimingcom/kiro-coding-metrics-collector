...
exports.uploadCommitStats = uploadCommitStats;
 ...
 */
async function uploadCommitStats(workspaceDir, commitSha) {
    ...
    try {
        ...
        await postWithRetry(apiConfig_1.STATS_URL, idempotencyKey, payload);
 ...
 */
function queryCommitStats(binary, cwd, commitSha) {
    return new Promise((resolve) => {
        ...
        const proc = (0, node_child_process_1.spawn)(binary, args, {
            cwd,
            env: { ...process.env },
        });
        let stdout = "";
...
async function queryCommitStatsWithRetry(binary, cwd, commitSha) {
    for (let attempt = 0; attempt <= NOTE_RETRY_COUNT; attempt++) {
        if (attempt > 0) {
            console.log(`[git-ai-kiro] Retrying stats query (${attempt}/${NOTE_RETRY_COUNT}) for ${commitSha.slice(0, 8)}...`);
        ...
        console.log(`[git-ai-kiro] Stats for ${commitSha.slice(0, 8)} show ${stats.git_diff_added_lines} added lines ` +
...
function getRepoName(workspaceDir) {
...
function generateIdempotencyKey(commitSha, machineId) {
...
async function postWithRetry(url, idempotencyKey, payload) {
    const body = JSON.stringify(payload);
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        if (attempt > 0) {
            const delay = BASE_DELAY_MS * Math.pow(2, attempt - 1);
            console.log(`[git-ai-kiro] Stats upload retry ${attempt}/${MAX_RETRIES} after ${delay}ms`);
            await sleep(delay);
        }
        try {
            ...
            if (statusCode >= 400 && statusCode < 500) {
                console.error(`[git-ai-kiro] Stats upload got HTTP ${statusCode}, not retrying`);
                return;
            }
            console.warn(`[git-ai-kiro] Stats upload got HTTP ${statusCode}, will retry`);
        }
        catch (err) {
...
function doPost(url, idempotencyKey, body) {
...
//# sourceMappingURL=statsUploader.js.map