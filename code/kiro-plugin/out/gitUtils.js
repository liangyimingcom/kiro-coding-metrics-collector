...
exports.findGitReposInDir = findGitReposInDir;
 ...
 * own working tree).
 */
function findGitReposInDir(dirPath) {
    const MAX_DEPTH = 6;
...
function getHookConfig() {
    const statsUrl = apiConfig_1.STATS_URL;
    let ignoreArgs = "";
    try {
 ...
 */
function canRunShOnWindows() {
    if (os.platform() !== "win32")
        return true;
    if (_shProbeResult !== null)
        return _shProbeResult;
    ...
    try {
        const gitExecPath = (0, node_child_process_1.spawnSync)("git", ["--exec-path"], {
            ...
            for (const shPath of shCandidates) {
                try {
                    if (fs.existsSync(shPath)) {
                        const result = (0, node_child_process_1.spawnSync)(shPath, ["-c", "echo ok"], {
                            timeout: 5000, encoding: "utf-8", windowsHide: true,
                            stdio: ["ignore", "pipe", "pipe"],
                        });
    ...
    try {
        const result = (0, node_child_process_1.spawnSync)("sh", ["-c", "echo ok"], {
            timeout: 5000, encoding: "utf-8", windowsHide: true,
            stdio: ["ignore", "pipe", "pipe"],
        });
        if (result.status === 0 && (result.stdout || "").trim() === "ok") {
    ...
    try {
        const gitVersion = (0, node_child_process_1.spawnSync)("git", ["--version"], {
            timeout: 5000, encoding: "utf-8", windowsHide: true,
            stdio: ["ignore", "pipe", "pipe"],
        });
 ...
 */
function canRunPowerShellHere(hooksDir) {
    if (os.platform() !== "win32")
        return true;
    if (_psProbeResult !== null)
    ...
    try {
        ...
        const result = (0, node_child_process_1.spawnSync)(exe, ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", probePath], { timeout: 10_000, encoding: "utf-8", windowsHide: true, stdio: ["ignore", "pipe", "pipe"] });
...
function findPowerShellExe() {
    ...
    try {
        // whereis fallback
        const result = (0, node_child_process_1.spawnSync)("where", ["powershell.exe"], {
 ...
 */
function notifyPowerShellBlocked() {
    if (_psWarningShown)
        return;
    _psWarningShown = true;
    const msg = "无权限执行PowerShell脚本，影响代码指标上报，请以管理员身份打开 PowerShell，执行以下命令修改执行策略：\n" +
    ...
    try {
        ...
        if (vscode?.window?.showWarningMessage) {
            ...
            vscode.window.showWarningMessage(msg, copyAction).then((choice) => {
