...
/// Test that AI attribution is preserved when a file is renamed (git mv) after AI edits.
/// This verifies the rename detection in from_just_working_log correctly remaps
/// old file paths to new file paths so stats correctly attribute AI lines.
#[test]
fn test_stats_ai_attribution_survives_file_rename() {
    let repo = TestRepo::new();

    // Initial commit with a human-written file
    let mut file = repo.filename("original.txt");
    file.set_contents(crate::lines!["line 1".human()]);
    repo.stage_all_and_commit("Initial commit").unwrap();

    // AI adds lines to the file (checkpoint records "original.txt")
    file.set_contents(crate::lines![
        "line 1".human(),
        "ai line 2".ai(),
        "ai line 3".ai(),
    ...
    // Rename the file AFTER AI checkpoint was recorded under old path
    repo.git(&["mv", "original.txt", "renamed.txt"]).unwrap();

    let commit = repo.stage_all_and_commit("Rename with AI edits").unwrap();

    let stats = stats_from_args(&repo, &["stats", &commit.commit_sha, "--json"]);

    // With --no-renames, git diff treats rename as delete old + add new,
    // so all lines in the new file are "added". The AI lines should still
    // be attributed to AI via the authorship log.
    assert_eq!(
        stats.ai_accepted, 2,
        "AI lines should be attributed as ai_accepted after rename"
    );
    // "line 1" is also counted as added (new file from git's perspective)
    assert_eq!(
        stats.human_additions, 1,
        "line 1 is a human addition (existed before but git sees it as new)"
    );
    assert_eq!(stats.git_diff_added_lines, 3, "all 3 lines are added (no-renames)");
}

/// Test that AI attribution is preserved when a file is moved to a subdirectory.
#[test]
fn test_stats_ai_attribution_survives_file_move() {
    let repo = TestRepo::new();
    ...
    // AI adds content (checkpoint records "src.txt")
    file.set_contents(crate::lines![
        "base".human(),
        "ai content".ai(),
    ]);

    // Move file to subdirectory AFTER AI checkpoint
    std::fs::create_dir_all(repo.path().join("subdir")).unwrap();
    repo.git(&["mv", "src.txt", "subdir/src.txt"]).unwrap();

    let commit = repo.stage_all_and_commit("Move file to subdir").unwrap();

    let stats = stats_from_args(&repo, &["stats", &commit.commit_sha, "--json"]);

    // With --no-renames, all lines in the moved file are "added"
    assert_eq!(
        stats.ai_accepted, 1,
        "AI line should be attributed as ai_accepted after move"
    );
    assert_eq!(
        stats.human_additions, 1,
        "base line is a human addition (git sees it as new)"
    );
    assert_eq!(stats.git_diff_added_lines, 2, "2 lines added (no-renames)");
}

/// Test that AI attribution is preserved when AI creates a NEW file and user renames it before commit.
/// This is different from the rename test above where the file already existed.
#[test]
fn test_stats_ai_new_file_then_user_rename() {
    let repo = TestRepo::new();
    ...
    // AI creates a brand new file
    let mut file = repo.filename("AiGenerated.java");
    file.set_contents(crate::lines![
        "// AI generated".ai(),
        "public class AiGenerated {".ai(),
        "    public static void main(String[] args) {".ai(),
        "        System.out.println(42);".ai(),
        "    }".ai(),
        "}".ai(),
    ]);
    ...
    std::fs::rename(
        repo.path().join("AiGenerated.java"),
        repo.path().join("Renamed.java"),
    ).unwrap();

    let commit = repo.stage_all_and_commit("AI file renamed by user").unwrap();

    let stats = stats_from_args(&repo, &["stats", &commit.commit_sha, "--json"]);

    // All 6 lines should be AI-attributed even though the file was renamed
    assert_eq!(
        stats.ai_accepted, 6,
        "All AI lines should be attributed after user rename"
    );
    assert_eq!(stats.human_additions, 0, "No human additions expected");
    assert_eq!(stats.git_diff_added_lines, 6, "6 lines added");
}

/// Test that AI attribution is preserved when AI creates a NEW file and user moves it to subdir.
#[test]
fn test_stats_ai_new_file_then_user_move_to_subdir() {
    let repo = TestRepo::new();
    ...
    // User moves the file to a subdirectory before committing
    std::fs::create_dir_all(repo.path().join("sub1")).unwrap();
    std::fs::rename(
        repo.path().join("AiGenerated.java"),
        repo.path().join("sub1").join("AiGenerated.java"),
    ).unwrap();

    let commit = repo.stage_all_and_commit("AI file moved by user").unwrap();

    // Debug: check commit output for git-ai messages
    for line in commit.stdout.lines() {
        if line.contains("[git-ai]") || line.contains("from_just") || line.contains("INSERTING") || line.contains("REMOVING") || line.contains("blob fallback") || line.contains("Disabling pathspec") || line.contains("Rename detected") {
            eprintln!("DEBUG new_move: {}", line);
        }
    }
    let att_files: Vec<String> = commit.authorship_log.attestations.iter()
        .map(|a| format!("{}(entries={})", a.file_path, a.entries.len())).collect();
    eprintln!("DEBUG new_move: attestation files = {:?}", att_files);

    let stats = stats_from_args(&repo, &["stats", &commit.commit_sha, "--json"]);

    assert_eq!(
        stats.ai_accepted, 6,
        "All AI lines should be attributed after user move"
    );
    assert_eq!(stats.human_additions, 0, "No human additions expected");
    assert_eq!(stats.git_diff_added_lines, 6, "6 lines added");
}
...
crate::reuse_tests_in_worktree!(
    ...
    test_stats_ai_attribution_survives_file_rename,
    test_stats_ai_attribution_survives_file_move,
    test_stats_ai_new_file_then_user_rename,
    test_stats_ai_new_file_then_user_move_to_subdir,
);
