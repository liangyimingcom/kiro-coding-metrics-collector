...
impl IgnoreMatcher {
    ...
    pub fn is_ignored(&self, path: &str) -> bool {
        ...
        // Split path into segments for simple directory name matching
        let segments: Vec<&str> = path.split('/').collect();

        self.patterns.iter().any(|pattern| match pattern {
            CompiledPattern::Glob(glob_pattern) => {
                let pattern_str = glob_pattern.as_str();
                // If the pattern is a simple name (no glob chars, no path separators),
                // also check if it matches any path segment (directory name matching).
                let is_simple_name =
                    !pattern_str.contains('*') && !pattern_str.contains('/') && !pattern_str.contains('?');
                if is_simple_name && segments.contains(&pattern_str) {
                    return true;
                }
                glob_pattern.matches(path) || glob_pattern.matches(filename)
            }
            CompiledPattern::Exact(pattern) => {
                let is_simple_name = !pattern.contains('/');
                if is_simple_name && segments.contains(&pattern.as_str()) {
                    return true;
                }
