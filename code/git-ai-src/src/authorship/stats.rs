...
pub fn get_git_diff_stats(
    ...
    // Use PatchParse profile (not NumstatParse) to preserve rename detection.
    // With --no-renames (NumstatParse), renamed files are counted as full delete+add,
    // inflating human_additions for lines that were simply carried over from the old file.
    ...
    for line in stdout.lines() {
        ...
        // With rename detection, filename may be "{old => new}" or "dir/{old => new}"
        let parts: Vec<&str> = line.split('\t').collect();
        if parts.len() >= 3 {
            // Check if this file should be ignored
            let filename = parts[2];
            // For renames, extract the new filename for ignore checking
            let effective_filename = if filename.contains(" => ") {
                // Handle formats like "{old => new}" or "dir/{old => new}"
                filename
                    .replace('{', "")
                    .replace('}', "")
                    .split(" => ")
            ...
            if should_ignore_file_with_matcher(&effective_filename, &ignore_matcher) {
                continue;
            }
...
/// Print AI code adoption rate summary for the entire repository.
/// adoption_rate = ai_additions / git_diff_added_lines × 100%
pub fn print_adoption_rate(stats: &CommitStats) {
    let total = stats.git_diff_added_lines;
    let ai = stats.ai_additions;
    let human = stats.human_additions;

    if total == 0 {
        println!("No code changes found in this repository.");
        return;
    }

    let rate = (ai as f64 / total as f64) * 100.0;

    println!("AI Code Adoption Rate");
    println!("─────────────────────────────────────────");
    println!(
        "  AI changed lines:      {:>8}",
        ai
