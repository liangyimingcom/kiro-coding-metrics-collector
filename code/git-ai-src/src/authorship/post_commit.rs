...
pub fn post_commit_with_final_state(
    ...
    // Persistent debug: write key data to .git/ai/post_commit_debug.log
    if let Ok(workdir) = repo.workdir() {
        let debug_path = workdir.join(".git").join("ai").join("post_commit_debug.log");
        let va_files: Vec<String> = working_va.attributions.keys().cloned().collect();
        let checkpoint_info: Vec<String> = parent_working_log.iter().map(|cp| {
            let files: Vec<String> = cp.entries.iter().map(|e| {
                format!("{}(la={},a={})", e.file, e.line_attributions.len(), e.attributions.len())
            }).collect();
            format!("kind={:?} entries={:?}", cp.kind, files)
        }).collect();
        let ts = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs();
        let _ = std::fs::OpenOptions::new().create(true).append(true).open(&debug_path).and_then(|mut f| {
            use std::io::Write;
            writeln!(f, "--- {} ---\ncommit={}\nparent={}\nfinal_state={}\nva_files={:?}\ncheckpoints={:?}\n",
                ts, commit_sha, parent_sha, final_state_override.is_some(), va_files, checkpoint_info)
        });
    }
    ...
    // Detect file renames between parent and commit. When files are renamed/moved,
    // pathspecs contain the OLD path (from checkpoint), but git diff --no-renames
    // reports the NEW path. Add renamed paths to pathspecs so collect_committed_hunks
    // can find the added lines under the new path.
    //
    // Strategy: First try git's rename detection. If that doesn't find renames
    // (e.g., file too small or content changed too much), check if any pathspec
    // files were deleted in this commit. If so, disable pathspec filtering entirely
    // so collect_committed_hunks can find all added files including renamed ones.
    let rename_map = if parent_sha != "initial" {
        repo.diff_rename_map(&parent_sha, &commit_sha).unwrap_or_default()
    } else {
        HashMap::new()
    };
    let mut has_unresolved_deletes = false;
    for new_path in rename_map.values() {
        pathspecs.insert(new_path.clone());
    }
    // Check if any pathspec files don't exist in the commit tree.
    // This catches both deleted files (potential renames) and new files that were
    // renamed/moved before commit (AI creates file A, user renames to B).
    for pathspec in &pathspecs {
        if rename_map.contains_key(pathspec) {
            continue;
        }
        let mut cat_args = repo.global_args_for_exec();
        cat_args.push("cat-file".to_string());
        cat_args.push("-e".to_string());
        cat_args.push(format!("{}:{}", commit_sha, pathspec));
        if crate::git::repository::exec_git(&cat_args).is_err() {
            has_unresolved_deletes = true;
            break;
        }
    }

    // If there are unresolved deletes (potential renames), disable pathspec filtering
    // so collect_committed_hunks can find all added files
    let effective_pathspecs = if has_unresolved_deletes {
        debug_log("Disabling pathspec filtering due to unresolved file deletes (potential renames)");
        None
    } else {
        Some(&pathspecs)
    };

    // Persistent debug: append pathspec info
    if let Ok(workdir) = repo.workdir() {
        let debug_path = workdir.join(".git").join("ai").join("post_commit_debug.log");
        let _ = std::fs::OpenOptions::new().append(true).create(true).open(&debug_path).and_then(|mut f| {
            use std::io::Write;
            writeln!(f, "pathspecs={:?}\nrename_map={:?}\nhas_unresolved={}\n", pathspecs, rename_map, has_unresolved_deletes)
        });
    }
