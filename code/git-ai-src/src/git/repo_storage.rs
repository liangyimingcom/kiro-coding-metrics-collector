    ...
    pub fn read_current_file_content(&self, file_path: &str) -> Result<String, GitAiError> {
        ...
        match fs::read(&file_path) {
            Ok(bytes) => {
                ...
                // Normalize CRLF to LF to match git's internal storage format.
                // Without this, Windows line endings cause every line to appear
                // as changed when comparing against working log content that was
                // already normalized (e.g., from dirty_files passed by the Kiro plugin).
                if content.contains("\r\n") {
                    Ok(content.replace("\r\n", "\n"))
                } else {
                    Ok(content)
