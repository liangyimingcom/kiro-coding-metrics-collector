...
impl Repository {
    ...
    /// Get rename mappings between two refs using `git diff -M1%`.
    /// Uses a very low similarity threshold (1%) to catch renames even when
    /// the file content has changed significantly (e.g., AI added many lines).
    /// Returns a HashMap of old_path -> new_path for renamed files.
    pub fn diff_rename_map(
        &self,
        from_ref: &str,
        to_ref: &str,
    ) -> Result<HashMap<String, String>, GitAiError> {
        ...
        args.push("--find-renames=1%".to_string()); // Very low threshold to catch renames with edits
        args.push("-z".to_string()); // NUL-separated output
        args.push(from_ref.to_string());
        args.push(to_ref.to_string());
        ...
        // With -z and --name-status, renames are output as:
        // R<score>\0<old_path>\0<new_path>\0
        // Other entries are: <status>\0<path>\0
        ...
        while i < parts.len() {
            ...
            if status.starts_with('R') || status.starts_with('C') {
                // Rename or copy: next two parts are old_path and new_path
                if i + 2 < parts.len() {
                    let old_path = String::from_utf8_lossy(parts[i + 1]).to_string();
                    let new_path = String::from_utf8_lossy(parts[i + 2]).to_string();
                    rename_map.insert(
                        crate::utils::normalize_to_posix(&old_path),
                        crate::utils::normalize_to_posix(&new_path),
                    );
                    i += 3;
                } else {
                    break;
                ...
                // Other status (A, D, M): next part is the path
                i += 2;
            }
        }
