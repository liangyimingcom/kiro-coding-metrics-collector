        ...
        let futures = files.iter().map(|file_path| {
            ...
            async move {
                ...
                .unwrap_or_else(|| {
                    ...
                    // Read from filesystem, normalizing CRLF to LF to match
                    // git's internal storage format (see read_current_file_content).
                    let raw = std::fs::read_to_string(&abs_path).unwrap_or_default();
                    if raw.contains("\r\n") {
                        raw.replace("\r\n", "\n")
                    } else {
                        raw
